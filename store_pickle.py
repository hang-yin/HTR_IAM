...
for i in ds.partitions:
    with open(f"{i}_signatures.pickle", 'wb') as output1:
        pickle.dump(ds.dataset[i]['dt'], output1)
    with open(f"{i}_labels.pickle", 'wb') as output2:
        pickle.dump(ds.dataset[i]['gt'], output2)
    print(f"{i} partition stored.")

    '''
    with h5py.File(source_path, "a") as hf:
        # does h5py store multi-dimensional variable length arrays? use pickle instead?
